...
tardec() { [ -z "$3" ] && echo "usage: tardec file.aaa pass" || openssl aes-256-cbc -salt -pbkdf2 -in ${1} -pass pass:${2} -d | tar xJ ; }
...
tardect() { [ -z "$3" ] && echo "usage: tardect file.txt pass" || topenssl aes-256-cbc -salt -pbkdf2 -in ${1} -pass pass:${2} -a -d | tar xJ ; } # tardec file pass
