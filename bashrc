...
alias dockerkill3='docker rm -f $(docker ps -aq) ; docker rmi $(docker images -q)'
alias dockerkill3='docker rm -f $(docker ps -aq) ; docker rmi $(docker images -q) ; docker volume rm $( docker volume ls -q )'
alias dockerkill4='docker rm -f $(docker ps -aq) ; docker rmi $(docker images -q) ; docker volume rm $( docker volume ls -q ) ; docker system prune -a --volumes --force ; docker builder prune -a --force'
...
alias emojis='for i in {9728..10175} {127744..130020} {129648..129750}; do printf "\\U$(printf '%08x' $i) "; done; echo '
